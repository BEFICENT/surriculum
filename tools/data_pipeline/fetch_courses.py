...
def main():
    ...
    parser.add_argument("--skip-coursepages", action="store_true", help="Skip running the course-page scraper after fetching.")
    ...
    if not args.skip_coursepages:
        ...
        # (The old CSV-based tools.legacy.update_credits remains available but is deprecated.)
        print("\nRunning tools.data_pipeline.scrape_coursepages to update credits in JSON files...\n")
        subprocess.run(
            [sys.executable, '-m', 'tools.data_pipeline.scrape_coursepages'],
            check=True,
        )
