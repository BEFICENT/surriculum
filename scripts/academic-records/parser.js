// Pure Academic Records HTML/PDF parsing, normalization, and reconciliation.
(function installAcademicRecordsParsing(root) {
    'use strict';

    function createAcademicRecordsParsing(dependencies) {
        const runtime = dependencies || {};
        const getGradePolicyDependency = typeof runtime.getGradePolicy === 'function'
            ? runtime.getGradePolicy : () => null;
        const DOMParser = runtime.DOMParser;
...
function getTranscriptGradePolicy() {
    try {
        return getGradePolicyDependency() || null;
    } catch (_) {
...
function parseAcademicRecordsPdf(pdfText) {
    ...
    const browserApi = createAcademicRecordsParsing({
        getGradePolicy: () => root && root.gradePolicy,
        DOMParser: root && root.DOMParser,
    });
    const namespace = root.SurriculumModules || (root.SurriculumModules = {});
    namespace.academicRecordsParsing = Object.freeze({
        create: createAcademicRecordsParsing,
        TRANSCRIPT_SEMESTER_SKIP_REASON: browserApi.TRANSCRIPT_SEMESTER_SKIP_REASON,
        getTranscriptGradePolicy: browserApi.getTranscriptGradePolicy,
        normalizeTranscriptGrade: browserApi.normalizeTranscriptGrade,
        inferTranscriptGradingBasis: browserApi.inferTranscriptGradingBasis,
        normalizeTranscriptGradeRecord: browserApi.normalizeTranscriptGradeRecord,
        canonicalTranscriptCourseCode: browserApi.canonicalTranscriptCourseCode,
        isExactTranscriptLangCourseCode: browserApi.isExactTranscriptLangCourseCode,
        suggestedTranscriptLanguageLevel: browserApi.suggestedTranscriptLanguageLevel,
        transcriptSelectedDegreePrograms: browserApi.transcriptSelectedDegreePrograms,
        transcriptLanguageTypeForProgram: browserApi.transcriptLanguageTypeForProgram,
        normalizeTranscriptSemester: browserApi.normalizeTranscriptSemester,
        transcriptSemesterIssueLabel: browserApi.transcriptSemesterIssueLabel,
        transcriptSemesterOrder: browserApi.transcriptSemesterOrder,
        makeTranscriptCandidate: browserApi.makeTranscriptCandidate,
        reconcileTranscriptCandidates: browserApi.reconcileTranscriptCandidates,
        parseAcademicRecords: browserApi.parseAcademicRecords,
        parseYokTranscript: browserApi.parseYokTranscript,
        parseAcademicRecordsPdf: browserApi.parseAcademicRecordsPdf,
    });
